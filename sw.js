...
self.addEventListener('fetch', e => {
  ...
  // network-first for navigations/HTML so updates show up immediately; cache-first for static assets
  if (req.mode === 'navigate' || (req.headers.get('accept') || '').includes('text/html')) {
    e.respondWith(
      fetch(req).then(res => { caches.open(CACHE).then(c => c.put(req, res.clone())).catch(() => {}); return res; })
